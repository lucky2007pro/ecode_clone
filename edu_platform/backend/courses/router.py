...
async def add_course(course_in: CourseCreate, db: AsyncSession = Depends(get_db), school_id=Depends(get_current_school_id), _=Depends(RequirePermissions([Permission.MANAGE_COURSES]))):
    return await create_course(db, course_in, school_id)
